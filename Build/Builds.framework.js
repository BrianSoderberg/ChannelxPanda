...
function __ConnectAndSendDataToDevice(deviceName, dataName) {
 var parsedDeviceName = UTF8ToString(deviceName);
 var parsedDataName = UTF8ToString(dataName);
 console.log("device: " + parsedDeviceName);
 console.log("data: " + parsedDataName);
 navigator.bluetooth.requestDevice({
  filters: [ {
   name: parsedDeviceName
  } ],
  optionalServices: [ "9255e426-8951-44c8-8fa2-9a754f7d841c" ]
 }).then(device => {
  ...
  return server.getPrimaryService("9255e426-8951-44c8-8fa2-9a754f7d841c");
 }).then(service => {
  console.log("> Found command service");
  return service.getCharacteristic("1d4531f3-c500-49b6-8af4-9a108c238a98");
 }).then(characteristic => {
  myBluetoothCharacteristic = characteristic;
  console.log("> Found characteristic");
  const intValue = parseInt(parsedDataName);
  console.log(intValue);
  const volumeLevel = Uint8Array.of(intValue & 255, intValue >> 8, 0);
  return characteristic.writeValue(volumeLevel);
 }).catch(error => {
  console.error(error);
 });
}

function __ConnectToDevice(deviceName) {
 var parsedDeviceName = UTF8ToString(deviceName);
 var parsedDataName = UTF8ToString(dataName);
 console.log("device: " + parsedDeviceName);
 console.log("data: " + parsedDataName);
...
function __FindBLEDevices() {
 navigator.bluetooth.requestDevice({
  acceptAllDevices: true,
  optionalServices: [ "battery_service" ]
 }).then(device => {}).catch(error => {
  console.error(error);
 });
}

function __FindNamedBLEDevice(deviceName) {
 var parsedDeviceName = UTF8ToString(deviceName);
 navigator.bluetooth.requestDevice({
  filters: [ {
   name: parsedDeviceName
...
function __ReadFromPairedDevice() {
 if (myBluetoothCharacteristic == null) return;
 return myBluetoothCharacteristic.readValue().then(value => {
  value = value.buffer ? value : new DataView(value);
  return value;
 });
}

function __WriteToPairedDevice(servoNumName, dataName) {
 if (myBluetoothCharacteristic == null) return;
 var parsedServoNumName = UTF8ToString(servoNumName);
 const servoNum = parseInt(parsedServoNumName);
 console.log("servo: " + parsedServoNumName);
 var parsedDataName = UTF8ToString(dataName);
 const intValue = parseInt(parsedDataName);
 const volumeLevel = Uint8Array.of(intValue & 255, intValue >> 8, servoNum);
 myBluetoothCharacteristic.writeValue(volumeLevel);
}
...
var asmLibraryArg = {
 ...
 "_ConnectAndSendDataToDevice": __ConnectAndSendDataToDevice,
 "_ConnectToDevice": __ConnectToDevice,
 "_FindBLEDevices": __FindBLEDevices,
 "_FindNamedBLEDevice": __FindNamedBLEDevice,
 "_ReadFromPairedDevice": __ReadFromPairedDevice,
 "_WriteToPairedDevice": __WriteToPairedDevice,
...
unexportedRuntimeFunction("MyData", false);

unexportedRuntimeFunction("MyData__user", false);
